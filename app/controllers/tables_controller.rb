class TablesController < ApplicationController
	...
	def extension
		...
		@table.table_girls.each do |table_girl|
			if table_girl.today_girl_id != nil
				if table_girl.named_status == 1
					@payment += @shop.hall_price
				elsif table_girl.named_status == 2
					@payment += @payment + @shop.name_price
				elsif table_girl.named_status == 3
	...
	private
		...
		def table_params
			params.require(:table).permit(:number,:today_id,:member,:time,:price,:set_time,:set_count,:payment,:name,:memo,:tax,nameds_attributes:[:today_girl_id,:table_id,:named_status])
		end
